...
MidiMessage::MidiMessage(int command)
    : content({static_cast<uchar>(command)}) {}

MidiMessage::MidiMessage(int command, int p1)
    : content({static_cast<uchar>(command), static_cast<uchar>(p1)}) {}

MidiMessage::MidiMessage(int command, int p1, int p2)
    : content({static_cast<uchar>(command), static_cast<uchar>(p1), static_cast<uchar>(p2)}) {}

MidiMessage::MidiMessage(Content content)
    : content(std::move(content)) {
}

// MidiMessage::getSize -- Return the size of the MIDI message bytes.
std::size_t MidiMessage::getSize() const {
    return content.size();
}

// MidiMessage::resizeToCommand -- Set the number of parameters if the
...
int MidiMessage::getTempoMicro() const {
    if (!isTempo()) {
        ...
        return (content[3] << 16) + (content[4] << 8) + content[5];
    }
}
...
bool MidiMessage::isNoteOff() const {
    if (getSize() != 3) {
        return false;
    } else if (!content.empty() && (content.at(0) & 0xf0) == 0x80) {
        return true;
    } else if (content.size() > 2 && ((content.at(0) & 0xf0) == 0x90) && (content.at(2) == 0)) {
        return true;
    } else {
        return false;
    }
...
bool MidiMessage::isNoteOn() const {
    if (getSize() != 3) {
        return false;
    } else if (!content.empty() && (content.at(0) & 0xf0) != 0x90) {
        return false;
    } else if (content[2] == 0) {
        return false;
    } else {
...
bool MidiMessage::isAftertouch() const {
    if (getSize() != 3) {
        return false;
    } else if (!content.empty() && (content.at(0) & 0xf0) != 0xA0) {
        return false;
    } else {
        return true;
    }
...
bool MidiMessage::isController() const {
    if (getSize() != 3) {
        return false;
    } else if (!content.empty() && (content.at(0) & 0xf0) != 0xB0) {
        return false;
    } else {
        return true;
    }
...
bool MidiMessage::isTimbre() const {
    if (!content.empty() && (content.at(0) & 0xf0) != 0xc0) {
        return false;
    } else if (getSize() != 2) {
        return false;
    } else {
...
bool MidiMessage::isPressure() const {
    if (!content.empty() && (content.at(0) & 0xf0) != 0xd0) {
        return false;
    } else if (getSize() != 2) {
        return false;
    } else {
...
bool MidiMessage::isPitchbend() const {
    if (!content.empty() && (content.at(0) & 0xf0) != 0xe0) {
        return false;
    } else if (getSize() != 3) {
        return false;
    } else {
...
// MidiMessage::getCommandNibble -- Returns the top 4 bits of the content[0]
//    entry, or -1 if there is not content[0].
int MidiMessage::getCommandNibble() const {
    if (content.empty()) {
        return -1;
    } else {
        return content.at(0) & 0xf0;
    }
}
...
//      content[0] entry, or -1 if there is not content[0].  Should be refined
...
void MidiMessage::setCommandByte(int value) {
    ...
    content[0] = static_cast<uchar>(value & 0xff);
}
...
void MidiMessage::setCommandNibble(int value) {
    ...
    if (value <= 0x0f) {
        content[0] = (content[0] & 0x0f) | ((uchar)((value << 4) & 0xf0));
    } else {
        content[0] = (content[0] & 0x0f) | ((uchar)(value & 0xf0));
    }
}
...
void MidiMessage::setChannelNibble(int value) {
    ...
    content[0] = (content[0] & 0xf0) | ((uchar)(value & 0x0f));
}
...
// MidiMessage::setContent --  Set the contents of MIDI bytes to the
//   input list of bytes.
void MidiMessage::setContent(Content const& otherContent) {
...
//    a meta-message type.
void MidiMessage::setMetaContent(std::vector<uchar> const& metaContent) {
    if (content.size() < 2) {
        // invalid message, so ignore request
        return;
    }
    if (metaContent.at(0) != 0xFF) {
    ...
    int dsize = (int) metaContent.size();
    if (dsize < 128) {
        content.push_back((uchar) dsize);
    } else {
        ...
        if (byte5) {
            content.push_back(byte5);
        }
        if (byte4) {
            content.push_back(byte4);
        }
        if (byte3) {
            content.push_back(byte3);
        }
        if (byte2) {
            content.push_back(byte2);
        }
        content.push_back(byte1);
    }
...
void MidiMessage::setTempoMicroseconds(int microseconds) {
    ...
    content[1] = 0x51;
    content[2] = 3;
    content[3] = (microseconds >> 16) & 0xff;
...
void MidiMessage::makeTimeSignature(int top, int bottom, int clocksPerClick,
    ...
    content[2] = 4;
    content[3] = 0xff & top;
    content[4] = 0xff & base2;
    content[5] = 0xff & clocksPerClick;
    content[6] = 0xff & num32ndsPerQuarter;
}
